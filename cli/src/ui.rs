...
pub fn draw(f: &mut Frame, explorer: &mut Explorer) {
    ...
    let editor_content = if let Some(content) = &explorer.active_file {
        let lines: Vec<Line> = content
            .lines()
            .map(|line| {
                let tokens = syntax::parse(line);
                let spans: Vec<Span> = tokens
                    .into_iter()
                    .map(|(token_type, text)| {
                        Span::styled(text.to_string(), token_type.to_style())
                    })
                    .collect();
                Line::from(spans)
            })
    ...
    let editor = editor_content.block(Block::default().title("Editor").borders(Borders::ALL));
    f.render_widget(editor, main_chunks[1]);

    let status_bar = Paragraph::new("[CTRL+S Save] [CTRL+Q Sair] [ENTER Abrir]").block(Block::default());
